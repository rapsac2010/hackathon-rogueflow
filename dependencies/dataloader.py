...
class ImglistToTensor(torch.nn.Module):
    ...
    of shape (CHANNELS x NUM_IMAGES x HEIGHT x WIDTH) in the range [0,1].
    Can be used as first transform for ``VideoFrameDataset``.
    """
    @staticmethod
    def forward(img_list: List[Image.Image]) -> 'torch.Tensor[CHANNELS, NUM_IMAGES, HEIGHT, WIDTH]':
        """
        ...
        Returns:
            tensor of size ``CHANNELS x NUM_IMAGES x HEIGHT x WIDTH``
        """
